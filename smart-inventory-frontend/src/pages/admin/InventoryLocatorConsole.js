    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const [stockData, whData, catData] = await Promise.all([
                inventoryStockService.getAll(), // Sửa lại tên hàm
                    ...
                    <table className="matrix-table">
                        <thead>
                            <tr>
                                ...
                                {isAdmin && <th style={{textAlign: 'right'}}>{t('stock.col_val') || 'ASSET VALUE'}</th>}
                            </tr>
                        </thead>
                        <tbody>
                            {loading ? (
                                <tr>
                                    <td colSpan={isAdmin ? 7 : 6} style={{textAlign: 'center', padding: '3rem', opacity: 0.5}}>
                                        {t('stock.init_radar') || 'INITIALIZING RADAR...'}
                                    </td>
                                </tr>
                            ) : filteredStocks.length > 0 ? (
                                filteredStocks.map(stock => {
                                    const isLowStock = stock.quantity <= 10; // Dummy low stock threshold
                                    const isExpiringSoon = stock.expiryDate && (new Date(stock.expiryDate) - new Date()) / (1000 * 60 * 60 * 24) < 30;
                                    
                                    const assetValue = (stock.quantity * (stock.basePrice || 0));
                                    ...
                                    return (
                                        <tr key={`${stock.productId}-${stock.warehouseId}-${stock.locationId}-${stock.batchNumber}`}>
                                            ...
                                            {isAdmin && (
                                                <td style={{textAlign: 'right'}}>
                                                    {stock.basePrice ? (
                                                        <div className="lcd-display lcd-val">
                                                            <span style={{fontSize: '0.7rem', opacity: 0.7}}>$</span>
                                                            {assetValue.toLocaleString('en-US', {minimumFractionDigits: 2})}
                                                        </div>
                                                    ) : (
                                                        <div className="lcd-locked" style={{justifyContent: 'flex-end', minWidth: '80px'}}>-</div>
                                                    )}
                                                </td>
                                            )}
                                        </tr>
                                ...
                                <tr>
                                    <td colSpan={isAdmin ? 7 : 6} style={{textAlign: 'center', padding: '3rem', color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)'}}>
                ...
                {isAdmin && filteredStocks.length > 0 && (
                    <div className="summary-telemetry">
                        <div className="summary-item">
                            <span className="summary-label">{t('stock.total_assets') || 'TOTAL SCANNED ASSETS'}</span>
                            <span className="summary-value-lcd">
                                <span style={{fontSize: '1rem', opacity: 0.5}}>$</span>
                                {filteredStocks.reduce((sum, s) => sum + (s.quantity * (s.basePrice || 0)), 0).toLocaleString('en-US', {minimumFractionDigits: 2})}
                            </span>
                        </div>
                    </div>
                )}
...
export default InventoryLocatorConsole;